...
class RegisterViewModel @Inject constructor(private val repository: FireBaseRepository) :
        ...
        password:String)
    {
     viewModelScope.launch {
         val error = if(fullName.isEmpty() || email.isEmpty() || phoneNumber.isEmpty() || password.isEmpty())
         {
             "Empty Strings"
         ...
         _userRegisterStatus.emit(Resource.Loading())
             ...
             val registrationResult = repository.registerUser(fullName = fullName, userEmail = email,phoneNumber = phoneNumber
